...
import vn.cinema.domain.booking.entity.*;
...
public class BookingService {
    ...
    public Page<BookingHistoryResponse> getBookingHistory(Long userId, Instant start, Instant end, int page, int size, BookingStatus status) {
        // check start, end
        Instant actualEnd = (end != null) ? end : Instant.now();
        Instant actualStart = (start != null) ? start : actualEnd.minus(Duration.ofDays(90));
        if (Duration.between(actualStart, actualEnd).toDays() > 90) {
            throw new BusinessRuleException(BusinessErrorCode.INVALID_REQUEST, "Khoảng thời gian tra cứu không được vượt quá 90 ngày");
        }

        Sort sort = Sort.by(Sort.Direction.DESC, "createdAt");
        Pageable pageable = PageRequest.of(page, size, sort);

        Page<Booking> bookingPage = bookingRepository.findByCustomerIdAndStatusAndCreatedAtBetween(
        ...
        return bookingPage.map(booking -> {
            ...
            String seatNames = bookingSeats.stream().map(BookingSeat::getSeatLabel)
            ...
            return BookingHistoryResponse.builder()
                    ...
                    .roomName(showtime.getAuditorium().getName())
                    .showtimeStat(showtime.getStartTime())
                    .seatNames(seatNames).build();
        });
    }
    ...
    public void cancelBooking(String bookingCode, Long userId) {
        // tìm booking từ bookingCode + userId
        Booking booking = bookingRepository.findByBookingCodeAndCustomerId(bookingCode, userId)
                .orElseThrow(() -> new ResourceNotFoundException(
                        BusinessErrorCode.RESOURCE_NOT_FOUND,
                        "Booking with ID: " + bookingCode));
        booking.ensureCancel();

        // check logic, khi booking đang PENDING
        if (booking.getStatus() == BookingStatus.PENDING) {
            releaseSeats(booking); // trả ghế
            booking.cancel();
        }
        else if (booking.getStatus() == BookingStatus.CONFIRMED) { // CONFIRMED thì trả ghế BOOKED -> AVAILABLE, đánh dấu refund_pending
            // check thời gian cho phép huỷ trước 45p showtime
            Showtime showtime = booking.getShowtime();
            Instant deadline = showtime.getStartTime().minus(45, ChronoUnit.MINUTES);

            if (!Instant.now().isBefore(deadline)) {
                throw new CancellationNotAllowedException(
                        "Booking can only be cancelled at least 45 minutes before showtime"
                );
            }

            // huỷ booking: CONFIRMED --> AVAILABLE
            booking.cancel();

            // vô hiệu hoá ticket
            ticketRepository.cancelTicketByBookingId(booking.getId(), TicketStatus.CANCELLED);

            // trả ghế
            List<ShowtimeSeat> showtimeSeats = booking.getBookingSeats().stream()
                    .map(BookingSeat::getShowtimeSeat)
                    .toList();
            showtimeSeats.forEach(ShowtimeSeat::releaseBooked);
            showtimeSeatRepository.saveAll(showtimeSeats);

            // tạo bản ghi refund ở trạng thái PENDING
            Payment payment = paymentRepository.findByBooking_IdAndStatus(booking.getId(), PaymentStatus.SUCCESS)
                    .orElseThrow(() -> new ResourceNotFoundException("Payment not found with bookingId: " + booking.getId()));
            Refund refund = Refund.builder()
                    .payment(payment)
                    .amount(payment.getAmount())
                    .status(RefundStatus.PENDING)
                    .reason("Khách hàng chủ động huỷ vé")
                    .build();
    ...
    // hàm trả ghế từ booking HELD
    private void releaseSeats(Booking booking) {
        List<ShowtimeSeat> showtimeSeats = booking.getBookingSeats().stream()
                .map(BookingSeat::getShowtimeSeat)
                .toList();

        showtimeSeats.forEach(ShowtimeSeat::release);
        showtimeSeatRepository.saveAll(showtimeSeats);
    }

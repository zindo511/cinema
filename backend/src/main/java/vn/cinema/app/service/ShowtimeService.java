...
import vn.cinema.app.dto.request.UpdateShowtimeRequest;
...
public class ShowtimeService {
    ...
    public ShowtimeDetailResponse createShowtime(CreateShowtimeRequest request) {
        ...
        Auditorium auditorium = findAuditorium(request.getAuditoriumId());
    ...
    // update showtime: khi nó ở trạng thái DRAFT
    @Transactional
    public void updateShowtime(Long showtimeId, UpdateShowtimeRequest request) {
        Showtime showtime = findShowtime(showtimeId);
        showtime.ensureShowtimeDraft();

        // xác định xem có sự thay đổi so với db (auditorium, base_price)
        boolean isAuditoriumChanged = request.getAuditoriumId() != null
                && !request.getAuditoriumId().equals(showtime.getAuditorium().getId());

        boolean isBasePriceChanged = request.getBasePrice() != null
                && request.getBasePrice().compareTo(showtime.getBasePrice()) != 0;

        // cập nhật movie, auditorium, startTime, base_price --> entity showtime
        boolean isMovieChanged = request.getMovieId() != null
                && !request.getMovieId().equals(showtime.getMovie().getId());
        if (isMovieChanged) {
            Movie movie = findMovie(request.getMovieId());
        ...
        // luôn tính lại end_time nếu như movie hoặc start_time đổi
        if (isMovieChanged || request.getStartTime() != null) {
            Instant endTime = showtime.getStartTime().plus(
                    Duration.ofMinutes(showtime.getMovie().getDurationMinutes())
            );
        ...
        // check auditorirum, có auditorium --> delete showtime_seat, nếu ko thì chỉ cần cập nhật base_price
        if (isAuditoriumChanged) {
            Auditorium auditorium = findAuditorium(request.getAuditoriumId());
            showtime.setAuditorium(auditorium);

            showtimeSeatRepository.deleteAllByShowtime_Id(showtimeId);
            List<Seat> activeSeats = seatRepository.findByAuditoriumIdAndIsActiveTrue(request.getAuditoriumId());
            createShowtimeSeats(showtime, activeSeats, showtime.getBasePrice());
        }
        else if (isBasePriceChanged) {
            List<ShowtimeSeat> currentSeats = showtimeSeatRepository.findAllByShowtime_Id(showtimeId);
            currentSeats.forEach(seat -> {
                BigDecimal newPrice = showtime.getBasePrice()
                        .multiply(seat.getSeat().getSeatType().getPriceMultiplier()).setScale(0, RoundingMode.HALF_UP);
                seat.setPrice(newPrice);
            });
            showtimeSeatRepository.saveAll(currentSeats);
        }
    }
    ...
    // tạo showtime_seat từ các seat active true

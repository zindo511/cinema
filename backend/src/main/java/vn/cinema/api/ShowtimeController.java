...
public class ShowtimeController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ShowtimeSeatMapResponse> getShowtimeDetails(@PathVariable Long id) {
        return ResponseEntity.ok(showtimeService.viewSeatMap(id));
    }

    @PreAuthorize("hasRole('ADMIN')")
    @PatchMapping("/{showtimeId}/update")
